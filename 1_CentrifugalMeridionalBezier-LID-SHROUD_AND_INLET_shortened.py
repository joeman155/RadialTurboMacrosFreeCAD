...
class Meridional:
	def __init__(self, obj):
		obj.Proxy = self
		obj.addProperty("App::PropertyFloat", "D2", "Dimensions", "Diameter of an impeller").D2=62.37
		obj.addProperty("App::PropertyFloat", "d0", "Dimensions", "Diameter of a hub").d0=20.110
		obj.addProperty("App::PropertyFloat", "D0", "Dimensions", "Suction diameter").D0=28.05
		obj.addProperty("App::PropertyFloat", "b2", "Dimensions", "Width of a outlet").b2=3.1
		obj.addProperty("App::PropertyFloat", "L", "Dimensions", "Length").L=40
		obj.addProperty("App::PropertyBool", "CylinricalBlades", "Type of blades", "Number of streamlines").CylinricalBlades=False
		obj.addProperty("App::PropertyPercent", "LePositionShroud", "Place of a Leading edge, %").LePositionShroud=30
		obj.addProperty("App::PropertyPercent", "LePositionHub", "Place of a Leading edge, %").LePositionHub=30
		obj.addProperty("App::PropertyPercent", "LePositionAverage", "Place of a Leading edge, %").LePositionAverage=30
	...
	def execute (self, obj):
		...
		r0 = obj.d0/2.
		R0 = obj.D0/2.
		gap = 0.6     # The gap between the shroud of the impeller and the lid
		th  = 1         # The thickness of the shroud
		th_lid = 2     # The thickness of lid.


		# Inlet
		inletP1 = FreeCAD.Vector(0, R0+th+gap+th_lid,0)
		inletP2 = FreeCAD.Vector(-30, R0+th+gap+th_lid,0)
		inletP3 = FreeCAD.Vector(-30, R0+th+gap,0)
		inletP4 = FreeCAD.Vector(0, R0+th+gap,0)


		# Create points:
		shroudP1 = FreeCAD.Vector(0, R0+th+gap,0)
		shroudP2 = FreeCAD.Vector(obj.L-th-gap,R0+th+gap,0)
		shroudP3 = FreeCAD.Vector(obj.L-th-gap,(R2+R0)/2.+th+gap,0)
		shroudP4 = FreeCAD.Vector(obj.L-th-gap,R2,0)

		
		shroud2P1 = FreeCAD.Vector(0, R0+th+gap+th_lid,0)
		shroud2P2 = FreeCAD.Vector(obj.L-th-gap-th_lid,R0+th+gap+th_lid,0)
		shroud2P3 = FreeCAD.Vector(obj.L-th-gap-th_lid,(R2+R0)/2.+th+gap+th_lid ,0)
		shroud2P4 = FreeCAD.Vector(obj.L-th-gap-th_lid,R2,0)
		shroud2P = [shroud2P1, shroud2P2, shroud2P3, shroud2P4]

		shroud2 = Part.BezierCurve()
		shroud2.setPoles(shroud2P)
		shroud2.toShape()
		shroud2Discret = shroud2.discretize(Number = 100)
...
#		aveCurveDiscret = aveCurve.discretize(Number = 100)
		...
		connector1 = Part.LineSegment(shroudP4, shroud2P4).toShape()
		connector2 = Part.LineSegment(shroud2P1, shroudP1).toShape()

		inlet1 = Part.LineSegment(inletP1, inletP2).toShape()
		inlet2 = Part.LineSegment(inletP2, inletP3).toShape()
		inlet3 = Part.LineSegment(inletP3, inletP4).toShape()
...
#			w = Part.Wire([inlet1,inlet2, shroud1.toShape(), shroud2.toShape(), outlet1, outlet2, hub1.toShape(), hub2.toShape(), Le1Curve.toShape(), Le2Curve.toShape()], closed = False)
		...
		w = Part.Wire([shroud.toShape(), connector1, shroud2.toShape(), inlet1, inlet2, inlet3], closed = False)
		

#		shroudSurface = shroud.toShape().revolve(FreeCAD.Vector(0,0,0), FreeCAD.Vector(1,0,0), 360)
#		shroud2Surface = shroud2.toShape().revolve(FreeCAD.Vector(0,0,0), FreeCAD.Vector(1,0,0), 360)
...
#		obj.Shape = Part.Compound([w, shroudSurface, shroud2Surface])
